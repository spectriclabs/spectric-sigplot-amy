...
(function () {
    ...
    var ColorMap = (window.ColorMap = function (colors, options) {
        ...
        for (var n = 0; n < ncolors - 2; n++) {
            ...
            this._addColor(
                this.interpolate(col1, col2, factorStep * colorBlockIndex)
            );
            colorBlockIndex += 1;
        }
    ...
    ColorMap.prototype = {
        ...
        _parseColors: function (colors) {
            for (var i = 0, c = colors.length; i < c; i++) {
                ...
                if (typeof color === "string") {
                    ...
                    colors[i].red = Math.floor(
                        Math.round(255 * (color.red / 100))
                    );
                    colors[i].green = Math.floor(
                        Math.round(255 * (color.green / 100))
                    );
                    colors[i].blue = Math.floor(
                        Math.round(255 * (color.blue / 100))
                    );
                }
            ...
            for (var i = 0, c = colors.length; i < c; i++) {
                ...
                if (!color.hasOwnProperty("pos")) {
                    ...
                    if (colorsWithNoStops) {
                        var stopSize =
                            (color.pos - lastStop) / colorsWithNoStops;
        ...
        setRange: function (low, high) {
            ...
            if (this._low !== low || this._high !== high) {
                ...
                this._high = high;
                this._fscale =
                    this.map.length / Math.abs(this._high - this._low);
            }
        },
